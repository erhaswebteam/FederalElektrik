...
namespace Grand.Services.Customers
{
    public partial class CustomerActionEventService : ICustomerActionEventService
    {
        ...
        {
            var action = from a in _customerActionRepository.Table
                         where a.Id == actionId
                         select a;
            ...
            if (query.Count() > 0)
            {
                if(query.FirstOrDefault().CreateDateUtc<action.FirstOrDefault().EndDateTimeUtc && query.FirstOrDefault().CreateDateUtc >= action.FirstOrDefault().StartDateTimeUtc)
                {
                    return false;
                }
                return true;
        ...
        protected bool Condition(CustomerAction action, Product product, string attributesXml, Customer customer, string currentUrl, string previousUrl)
        {
            ...
            foreach (var item in action.Conditions)
            {
                ...
                if (item.CustomerActionConditionType == CustomerActionConditionTypeEnum.UrlReferrer)
                {
                    ...
                    cond = previousUrl.Contains(item.UrlReferrer.Select(x => x.Name).FirstOrDefault());
                }
