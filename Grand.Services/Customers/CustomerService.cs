...
namespace Grand.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public virtual Customer GetCustomerByEmailORVatNumber(string emailORVatnumber)
        {
            if (string.IsNullOrWhiteSpace(emailORVatnumber))
                return null;
            return _customerRepository.Table.Where(x => x.Email == emailORVatnumber || x.GenericAttributes.Any(y => y.Key == "VatNumber" && y.Value == emailORVatnumber) == true).FirstOrDefault();
        }

        public virtual List<Customer> GetMyRegisterDealer(string customerId, string bolge)
        {
            if (string.IsNullOrWhiteSpace(bolge))
                return null;
            return _customerRepository.Table.Where(x => x.Id != customerId && x.CustomerRoles.Any(y => y.SystemName == bolge)).ToList();
        }
        ...
        public virtual Customer GetCustomerByUsername(string username)
        {
            ...
            var _lowerResult = _customerRepository.Table.Where(x => x.Username.ToLower() == username.ToLower()).FirstOrDefault();
            if (_lowerResult != null)
                return _lowerResult;
            else
                return _customerRepository.Table.Where(x => x.Username == username).FirstOrDefault();
        }
        ...
        public virtual void UpdateCustomer(Customer customer)
        {
            ...
            var update = Builders<Customer>.Update
                ...
                .Set(x => x.Username, string.IsNullOrEmpty(customer.Username) ? "" : customer.Username)
        ...
        public virtual void UpdateAddress(Address address)
        {
            ...
            var update = Builders<Customer>.Update
                ...
                .Set(x => x.Addresses.ElementAt(-1).GenericAttributes, address.GenericAttributes);
        ...
        public virtual void RemoveBillingAddress(string customerId)
        {
            var builder = Builders<Customer>.Filter;
            var filter = builder.Eq(x => x.Id, customerId);
            var update = Builders<Customer>.Update
                .Set(x => x.BillingAddress, null);
            var result = _customerRepository.Collection.UpdateOneAsync(filter, update).Result;
