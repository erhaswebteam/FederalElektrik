...
namespace Grand.Services.Topics
{
    ...
    public partial class TopicService : ITopicService
    {
        ...
        public virtual List<Topic> GetTopicBySystemName(string systemName)
        {
            if (String.IsNullOrEmpty(systemName))
                return null;
        ...
        public virtual Topic GetTopicBySeoName(string seoName)
        {
            if (String.IsNullOrEmpty(seoName))
                return null;

            var query = _topicRepository.Table;
            query = query.Where(t => t.SeName.ToLower() == seoName.ToLower());
            var topics = query.ToList();
            return topics.FirstOrDefault();
        }
