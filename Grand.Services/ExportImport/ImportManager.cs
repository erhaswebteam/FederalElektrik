...
namespace Grand.Services.ExportImport
{
    ...
    public partial class ImportManager : IImportManager
    {
        ...
        public ImportManager(IProductService productService,
            ...
            IMeasureService measureService, ICustomerService customerService,
        ...
        public virtual void ImportCustomerQuotaFromXlsx(Stream stream)
        {
            using (var xlPackage = new ExcelPackage(stream))
            {
                // get the first worksheet in the workbook
                ...
                //property array
                //the columns
                var properties = new List<PropertyByName<CustomerProductQuotaModel>>();
                var poz = 1;
                while (true)
                {
                    try
                    {
                        ...
                        poz += 1;
                        properties.Add(new PropertyByName<CustomerProductQuotaModel>(cell.Value.ToString().ToLower()));
                    }
                    catch
                    {
                        break;
                ...
                var manager = new PropertyManager<CustomerProductQuotaModel>(properties.ToArray());
                var templates = _manufacturerTemplateService.GetAllManufacturerTemplates();
                ...
                while (true)
                {
                    ...
                    var customerUsername = manager.GetProperty("kullanýcý adý") != null ? manager.GetProperty("kullanýcý adý").StringValue : string.Empty;
                    if (!string.IsNullOrEmpty(customerUsername))
                    {
                        Customer customer = _customerService.GetCustomerByUsername(customerUsername);
                        if (customer != null)
                        {
                            string _productId = "";
                            foreach (var property in manager.GetProperties)
                            {
                                ...
                                {
                                    case "ürün id":
                                        _productId = property.StringValue;
                                        break;
                                    case "kota":
                                        if(customer.CustomerRoles.Where(x=>x.SystemName== "Administrators" || x.SystemName== "BolgeMudurluk").Any())
                                        {
                                            var _productQuota = customer.GenericAttributes.Where(x => x.Key == _productId).FirstOrDefault();
                                            _genericAttributeService.SaveAttribute(customer, _productId, "100000", _storeContext.CurrentStore.Id);
                                        }
                                        else
                                        {
                                            var _productQuota = customer.GenericAttributes.Where(x => x.Key == _productId).FirstOrDefault();
                                            _genericAttributeService.SaveAttribute(customer, _productId, property.StringValue, _storeContext.CurrentStore.Id);
                                        }
