...
namespace Grand.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        public virtual byte[] ExportProductExportModelToXlsx(IEnumerable<ProductExportModel> products)
        {

            var properties = new[]
            {
            new PropertyByName<ProductExportModel>("SERVİS GRUBU", p => p.Role),
                new PropertyByName<ProductExportModel>("ÜRÜN", p => p.ProductName),
                new PropertyByName<ProductExportModel>("SKU", p => p.SKU),
                new PropertyByName<ProductExportModel>("ÜRÜN KATEGORİLERİ", p => p.ProductCategory),
                    new PropertyByName<ProductExportModel>("DESİ", p => p.Desi),
                new PropertyByName<ProductExportModel>("STOK", p => p.Stock),
                 new PropertyByName<ProductExportModel>("AKTİFLİK", p => p.IsActive),
                   new PropertyByName<ProductExportModel>("KDV ORANI", p => p.TaxRate),
                     //new PropertyByName<ProductExportModel>("FİYAT KDV'SİZ", p => p.PriceExl),
                       new PropertyByName<ProductExportModel>("PUAN", p => p.PriceInc),
                       new PropertyByName<ProductExportModel>("YAYIN TARİHİ", p => p.CreatedDate),
            };
        ...
        public virtual byte[] ExportOrderProductToXlsx(IList<ProductOrderModel> orders)
        {
            //property array
            var properties = new[]
            {
                    new PropertyByName<ProductOrderModel>("Kullanıcı Adı", p=>p.Username),
                    new PropertyByName<ProductOrderModel>("Cari Kod", p=>p.CariKod),
                    new PropertyByName<ProductOrderModel>("Ön Talep Numarası",p=>p.OrderNo),
                    new PropertyByName<ProductOrderModel>("SKU",p=>p.SKU),
                    new PropertyByName<ProductOrderModel>("Ürün Adı",p=>p.ProductName),
                    new PropertyByName<ProductOrderModel>("Adet",p=>p.Quantity),
                    new PropertyByName<ProductOrderModel>("Birim Fiyat",p=>p.BirimFiyati),
                    new PropertyByName<ProductOrderModel>("Kdv Oranı",p=>p.TaxRate),
                    new PropertyByName<ProductOrderModel>("Toplam",p=>p.Total),
                    new PropertyByName<ProductOrderModel>("Ad",p=>p.ShipmentName),
                    new PropertyByName<ProductOrderModel>("Soyad",p=>p.ShipmentSurname),
                    new PropertyByName<ProductOrderModel>("Teslimat Adresi",p=>p.ShipmentAddress),
                    new PropertyByName<ProductOrderModel>("Teslimat İlçe",p=>p.ShipmentTown),
                    new PropertyByName<ProductOrderModel>("Teslimat İl",p=>p.ShipmentCity),
                    new PropertyByName<ProductOrderModel>("Telefon",p=>p.Phone),
                    new PropertyByName<ProductOrderModel>("Sipariş Tarihi",p=>p.CreatedDate),
            };
        ...
        public virtual byte[] ExportOrderForAdminToXlsx(IList<ProductOrderModel> orders)
        {
            //property array
            var properties = new[]
            {
                    //new PropertyByName<ProductOrderModel>("Sipariş GuidNo", p=>p.OrderGuidNo),
                    new PropertyByName<ProductOrderModel>("Sipariş Numarası",p=>p.OrderNo),
                    new PropertyByName<ProductOrderModel>("SKU",p=>p.SKU),
                    new PropertyByName<ProductOrderModel>("Ürün Adı",p=>p.ProductName),
                    new PropertyByName<ProductOrderModel>("Adet",p=>p.Quantity),
                    new PropertyByName<ProductOrderModel>("Birim Fiyatı",p=>p.BirimFiyati),
                    new PropertyByName<ProductOrderModel>("Tutar",p=>p.TotalIncPrice),
                    new PropertyByName<ProductOrderModel>("Rol ve Bölge",p=>p.UserRole),
                    new PropertyByName<ProductOrderModel>("Müşteri No", p=>p.Username),
                    new PropertyByName<ProductOrderModel>("EMail", p=>p.Email),
                    new PropertyByName<ProductOrderModel>("Ad Soyad",p=>p.CompanyName),
                    new PropertyByName<ProductOrderModel>("Ödeme Tipi",p=>p.PaymentType),
                    new PropertyByName<ProductOrderModel>("Kargo Durumu",p=>p.ShippingStatus),
                    new PropertyByName<ProductOrderModel>("Sipariş Durumu",p=>p.OrderStatus),
                    new PropertyByName<ProductOrderModel>("Sipariş Tarihi",p=>p.CreatedDate),
            };
        ...
        public virtual byte[] ExportAllCustomersToXlsx(IList<Customer> customers)
        {
            var properties = new[]
           {
                    new PropertyByName<Customer>("Rolü", p => GetCustomerRolesNames(p.CustomerRoles.ToList(),",")),
                    new PropertyByName<Customer>("Kullanıcı Adı",p=>p.Username),
                    //new PropertyByName<Customer>("Şifre",p=>p.Password),
                    new PropertyByName<Customer>("Ad Soyad", p=>p.GetFullName()),
                    new PropertyByName<Customer>("Ünvan", p=>p.GenericAttributes.Where(x=>x.Key == "Company").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "Company").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("Vergi No", p=>p.GenericAttributes.Where(x=>x.Key == "VatNumber").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "VatNumber").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("Email",p=>p.Email),
                    new PropertyByName<Customer>("Telefon",p=>p.GenericAttributes.Where(x=>x.Key == "Phone").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "Phone").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("Kayıt Tarihi",p=>p.CreatedOnUtc.ToString(string.Format("dd/MM/yyyy HH:mm:ss"))),
                    new PropertyByName<Customer>("Aktif / Pasif",p=>p.Active),
                    new PropertyByName<Customer>("Yüklenen Puan",p=>p.GenericAttributes.Where(x=>x.Key == "Earn").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "Earn").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("Harcanan Puan",p=>p.GenericAttributes.Where(x=>x.Key == "Spend").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "Spend").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("İptal / İade",p=>p.GenericAttributes.Where(x=>x.Key == "Cancel").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "Cancel").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("Transfer Edilen Puan",p=>p.GenericAttributes.Where(x=>x.Key == "TransferSpend").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "TransferSpend").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("Transfer Gelen Puan",p=>p.GenericAttributes.Where(x=>x.Key == "TransferEarn").FirstOrDefault() !=null ? p.GenericAttributes.Where(x=>x.Key == "TransferEarn").FirstOrDefault().Value : string.Empty),
                    new PropertyByName<Customer>("Kullanılabilir Puan",p=>p.SystemName),
            };

            return ExportToXlsx(properties, customers);
        }
        public virtual byte[] ExportWishlistToXlsx(List<ShoppingCartItemModel> customers)
        {
            var properties = new[]
          {
                new PropertyByName<ShoppingCartItemModel>("Müşteri", p => p.Email),
                    new PropertyByName<ShoppingCartItemModel>("Ürün", p => p.ProductName),
                    new PropertyByName<ShoppingCartItemModel>("Ürün Nitelikleri",p=>p.AttributeInfo),
                    //new PropertyByName<Customer>("Şifre",p=>p.Password),
                    new PropertyByName<ShoppingCartItemModel>("Adet", p=>p.Quantity),
        ...
        public virtual byte[] ExportCustomerProductsQuotaToXlsx(List<CustomerProductQuotaModel> customersQuota)
        {

            var properties = new[]
            {
                new PropertyByName<CustomerProductQuotaModel>("Kullanıcı Adı", c => c.Username),
                new PropertyByName<CustomerProductQuotaModel>("Ad Soyad", c=>c.NameSurname),
                new PropertyByName<CustomerProductQuotaModel>("Ürün Adı", c=>c.ProductName),
                new PropertyByName<CustomerProductQuotaModel>("Ürün ID", c=>c.ProductId),
                new PropertyByName<CustomerProductQuotaModel>("Kota", c=>c.Quota),
            };

            return ExportToXlsx(properties, customersQuota);
        }
