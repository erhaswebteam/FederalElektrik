...
namespace Grand.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        public virtual IPagedList<Order> SearchOrders(string storeId = "",
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, string name = "", string companyName = "", string RoleName = "")
        {
            ...
            if (!string.IsNullOrEmpty(companyName))
                query = query.Where(x => x.LastName.ToLower().Contains(companyName.ToLower()));
            ...
            if (!string.IsNullOrEmpty(RoleName))
            {
                ...
                var customerList = _customerRepository.Table.Where(x => x.CustomerRoles.Any(r => r.Id == RoleName));

                //  var _tempCutomer = Builders<Customer>.Filter.ElemMatch(z => z.CustomerRoles, r => r.Id == RoleName);
                //  query = query.()
                //  var order = _orderRepository.Table;
                //  customerList = _customerRepository.Table.Take(order, x => x.CustomerRoles.Any(r => r.Id == RoleName), y => y.CustomerId, (x, y) => y);

                IMongoQueryable<Order> _tempQuery = (from q in _orderRepository.Table
                                                     join c in customerList
                                                     on q.CustomerId equals c.Id
                                                     where c.CustomerRoles.AsQueryable().Select(x => x.Id == RoleName).FirstOrDefault()
                                                     select q);
