    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            string reservationId = "", string parameter = "", string duration = "")
        {
            ...
            foreach (var cat in product.ProductCategories)
            {
                ...
                if (category == "625fe99602846a0418a0c359" || cat.CategoryId == "625fe99602846a0418a0c359")
                {
                    isCheque = true;
                    break;
                }
            ...
            if (isCheque)
            {
                ...
                foreach (var item in cart)
                {
                    ...
                    foreach (var cat in prdk.ProductCategories)
                    {
                        ...
                        {
                            warnings.Add("Lütfen alýþveriþ çekleri için ayrý sepet oluþturunuz.");
                            return warnings;
                        }
                    }
                }
            ...
            else
            {
                foreach (var item in cart)
                {
                    ...
                    foreach (var cat in prdk.ProductCategories)
                    {
                        ...
                        if (category == "625fe99602846a0418a0c359")
                        {
                            warnings.Add("Sepetinizde alýþveriþ çeki bulunuyor, lütfen alýþveriþ çekleri için ayrý sepet oluþturunuz.");
                            return warnings;
                        }
                    }
                }
            ...
            if (shoppingCartItem != null && product.ProductType != ProductType.Reservation)
            {
                ...
                //YENÝ STOK KONTROLÜ
                if (!StockCheckHelper.CheckStock(product, shoppingCartItem.Quantity, attributesXml) && !warnings.Any(x => x == _localizationService.GetResource("ShoppingCart.OutOfStock")))
                    warnings.Add(_localizationService.GetResource("ShoppingCart.OutOfStock"));
