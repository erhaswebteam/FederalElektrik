...
namespace Grand.Services.Orders
{
    ...
    public partial class RewardPointsService: IRewardPointsService
    {
        ...
        public decimal GetRewardPointsBalance(string customerId, string storeId)
        {
        ...
        public RewardPointsHistory AddRewardPointsHistory(string customerId, decimal points, string storeId,  string message = "",
        ...
        public virtual void RemoveRewardPointsHistory(string customerId, string storeId)
        {
            var query = _rphRepository.Table;
            if (!String.IsNullOrEmpty(customerId))
                query = query.Where(rph => rph.CustomerId == customerId);
            if (!_rewardPointsSettings.PointsAccumulatedForAllStores)
                query = query.Where(rph => rph.StoreId == storeId);
            query.ToList().ForEach(x => { _rphRepository.Delete(x); });
        }

        public virtual bool RemoveOldPointAndAddNewPoints(string CustomerId, decimal Point, string storeId, string message = null)
        {
            //Kullanıcının geçmişi siliniyor...
            RemoveRewardPointsHistory(CustomerId, storeId);
            //Son aktif puanı sisteme yazılıyor...
            AddRewardPointsHistory(CustomerId, Point, storeId, message);
