    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer, bool notifyStoreOwner)
        {
                ...
                && notifyCustomer)
            {
                ...
                if (orderCancelledCustomerNotificationQueuedEmailId > 0)
                {
                    _orderService.InsertOrderNote(new OrderNote
                    {
                        Note = "\"Sipariþ Ýptal Edildi\" kullanýcýya mail gönderildi.",
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow,
                        OrderId = order.Id,
                    });
                ...
                && notifyStoreOwner)
            {
                ...
                if (orderCancelledStoreOwnerNotificationQueuedEmailId > 0)
                {
                    _orderService.InsertOrderNote(new OrderNote
                    {
                        Note = "\"Sipariþ Ýptal Edildi\" kullanýcý tarafýndan...",
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    if (!processPaymentRequest.IsRecurringPayment)
                    {
                        ...
                        foreach (var sc in details.Cart)
                        {
                            ...
                            // Lowering the quota Customer 

                            var _productQuota = _workContext.CurrentCustomer.GenericAttributes.Where(x => x.Key == sc.ProductId).FirstOrDefault();
                            if (_productQuota != null)
                            {
                                _productQuota.Value = (int.Parse(_productQuota.Value) - sc.Quantity).ToString();
                                _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, sc.ProductId, _productQuota.Value, _storeContext.CurrentStore.Id);
                            }
                            ...
                            //YENÝ STOK KONTROLÜ
                            if (!StockCheckHelper.CheckStock(product, sc.Quantity, sc.AttributesXml))
                                throw new Exception(product.Name + " için yeterli stok bulunamadý. Lütfen ürünü sepetinizden çýkardýktan sonra tekrar oluþturunuz.");
                        ...
                        order.GenericAttributes.Add(new GenericAttribute() { Key = "WebshopDesiTotal", Value = _workContext.CurrentCustomer.GenericAttributes.Where(x => x.Key == "WebshopDesiTotal").FirstOrDefault().Value });
                        order.GenericAttributes.Add(new GenericAttribute() { Key = "SadakatDesiTotal", Value = _workContext.CurrentCustomer.GenericAttributes.Where(x => x.Key == "SadakatDesiTotal").FirstOrDefault().Value });
        ...
        public virtual void DeleteOrder(Order order)
        {
            ...
            if (order.OrderStatus != OrderStatus.Cancelled)
            {
                ...
                //// Stok sistemi yapýldýðý için kapatýlmýþtýr.
                //Adjust inventory
                //foreach (var orderItem in order.OrderItems)
                //{
                //    var product = _productService.GetProductById(orderItem.ProductId);
        ...
        public virtual void Ship(Shipment shipment, bool notifyCustomer)
        {
            ...
            {
                //Mail bildirimi
                _workflowMessageService.SendShipmentSentCustomerNotification(shipment, order.CustomerLanguageId);


                //Sms Bildirimi
                SendSMS_RequestModel reqSmsModel = new SendSMS_RequestModel();

                reqSmsModel.message = string.Format("Sn. {0} {1}, {2} nolu siparisiniz kargoya teslim edilmistir. Takip Numaranýz: {3}\nKargo Takip URL Adresi: https://www.yurticikargo.com/tr/online-servisler/gonderi-sorgula?code={4} Tiklayiniz.", order.ShippingAddress.FirstName, order.ShippingAddress.LastName, order.OrderNumber, shipment.TrackingNumber);
                reqSmsModel.number = order.ShippingAddress.GSM.Replace("(", "").Replace(")", "").Replace(" ", "").TrimStart('0'); ;
                ...
                if (postAPIData.SendSMS("http://erhassms.erhas.net/api/smsgateway/SendSMS", reqSmsModel))
                {
                    ...
                    _orderService.InsertOrderNote(new OrderNote
                    {
                        Note = "Kargoya verildi bilgisi Mail ve SMS olarak iletildi.",
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow,
                        OrderId = order.Id,
                    });
        ...
        public virtual void CancelOrder(Order order, bool notifyCustomer, bool notifyStoreOwner = false)
        {
            ...
            _orderService.InsertOrderNote(new OrderNote
            {
                Note = "Sipariþ Kullanýcý tarafýndan iptal edildi",
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow,
                OrderId = order.Id,
