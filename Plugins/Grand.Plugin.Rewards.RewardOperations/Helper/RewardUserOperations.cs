...
namespace Grand.Plugin.Rewards.RewardOperations.Helper
{
    public class RewardUserOperations : BasePlugin
    {
        ...
        public GetCustomerResponseModel GetCustomer(string Username, string Password)
        {
            ...
            try
            {
                var _rewardOperationsSettings = _settingService.LoadSetting<RewardOperationsSettings>();
        ...
        /// </summary>
        /// <returns></returns>
        public GetCustomerResponseModel GetCustomerByUsername(string Username)
        {
            GetCustomerResponseModel result = new GetCustomerResponseModel();
            ...
            try
            {
                ...
                string address = _rewardOperationsSettings.AddressGetUserByUsername;
                string projectPreChar = _rewardOperationsSettings.ProjectPreChars;
                string token = _rewardOperationsSettings.ApiToken;
        ...
        public SetUserPasswordRes SetCustomerPassword(string Username, string Password)
        {
            SetUserPasswordRes result = new SetUserPasswordRes();

            try
            {
                var _rewardOperationsSettings = _settingService.LoadSetting<RewardOperationsSettings>();

                string address = _rewardOperationsSettings.AddressSetUserPassword;
                string projectPreChar = _rewardOperationsSettings.ProjectPreChars;
                string token = _rewardOperationsSettings.ApiToken;

                SetUserPasswordReq model = new SetUserPasswordReq
                {
                    Token = token,
                    ProjectPreChars = projectPreChar,
                    Username = Username,
                ...
                try
                {
                    result = SendRequest<SetUserPasswordRes>(address, request);
                }
                catch (Exception ex)
                {
                    result.ErrorCode = -999;
