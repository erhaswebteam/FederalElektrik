...
namespace Grand.Plugin.Shipping.ByWeight.Controllers
{
    ...
    public class ShippingByWeightController : BaseShippingController
    {
        ...
        public IActionResult AddTax()
        {
            ...
            var sbw = _shippingByWeightService.GetAll();
            if (sbw == null)
                //No record found with the specified id
                return RedirectToAction("Configure");

            foreach (var tax in sbw)
            {
                tax.AdditionalFixedCost = Math.Round((tax.AdditionalFixedCost * 1.18M) * 10, 2);
                _shippingByWeightService.UpdateShippingByWeightRecord(tax);
            }
