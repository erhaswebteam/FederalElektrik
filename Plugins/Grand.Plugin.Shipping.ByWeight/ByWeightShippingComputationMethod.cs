...
namespace Grand.Plugin.Shipping.ByWeight
{
    public class ByWeightShippingComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        public GetShippingOptionResponse GetShippingOptions(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            decimal webshopDesi = 0;
            decimal sadakatDesi = 0;
            foreach (var packageItem in getShippingOptionRequest.Items)
            {
                ...
                var prdk = _productService.GetProductById(packageItem.ShoppingCartItem.ProductId);
                if (prdk != null)
                {
                    if (prdk.ProductCategories.Any(x => x.CategoryId == "63205c270f819e2630cac7c1" || x.CategoryId == "63205c150f819e2630cabb8a"))
                        webshopDesi += (prdk.Weight * packageItem.ShoppingCartItem.Quantity);
                    else
                        sadakatDesi += (prdk.Weight * packageItem.ShoppingCartItem.Quantity);
                }

                //shippingTotal = webshopDesi + sadakatDesi;
            }
            ...
            {
                decimal? webshopRate = GetRate(subTotal, webshopDesi, shippingMethod.Id, storeId, warehouseId, countryId, stateProvinceId, zip);
                decimal? sadakatRate = GetRate(subTotal, sadakatDesi, shippingMethod.Id, storeId, warehouseId, countryId, stateProvinceId, zip);
                if (webshopRate.HasValue || sadakatRate.HasValue)
                {
                    if (!webshopRate.HasValue)
                        webshopRate = 0;
                    
                    if (!sadakatRate.HasValue)
                        sadakatRate = 0;

                    var _savedWebshopDesiTotal = _workContext.CurrentCustomer.GenericAttributes.Where(x=>x.Key== "WebshopDesiTotal").FirstOrDefault();
                    var _savedSadakatDesiTotal = _workContext.CurrentCustomer.GenericAttributes.Where(x => x.Key == "SadakatDesiTotal").FirstOrDefault();
                    if (_savedWebshopDesiTotal != null)
                        _workContext.CurrentCustomer.GenericAttributes.Remove(_savedWebshopDesiTotal);
                    if (_savedSadakatDesiTotal != null)
                        _workContext.CurrentCustomer.GenericAttributes.Remove(_savedSadakatDesiTotal);

                    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, "WebshopDesiTotal", webshopRate.ToString(), storeId);
                    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, "SadakatDesiTotal", sadakatRate.ToString(), storeId);
                    ...
                    shippingOption.Rate = webshopRate.Value + sadakatRate.Value;
