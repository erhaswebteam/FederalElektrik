    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual IActionResult Index([FromServices] IShoppingCartService shoppingCartService, [FromServices] IProductService productService)
        {
            ...
            foreach (ShoppingCartItem sci in cart)
            {
                ...
                //puanın para kat sayısı ayarlardan getiriliyor...
                var setting = _settingService.GetSetting("payment.moneypointfactor");

                //puanın paraya dönüşmesi için katsayısı aktarılıyor...
                sci.MoneyPointFactor = int.Parse(setting.Value);
            }
        ...
        protected JsonResult OpcLoadStepAfterShippingMethod(List<ShoppingCartItem> cart)
        {
            ...
            if (isPaymentWorkflowRequired)
            {
                ...
                //Customer Point
                int rewardPoints = Convert.ToInt32( _processService.GetCustomerActualPoint(_workContext.CurrentCustomer.Username));
                paymentMethodModel.CustomerMoney = rewardPoints;
        ...
        public virtual IActionResult OpcSaveBilling(IFormCollection form)
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(billingAddressId))
                {
                    ...
                    address.VatNumber = form["BillingNewAddress.IdentityNumber"];
                    _workContext.CurrentCustomer.BillingAddress = address;
                    address.CustomerId = _workContext.CurrentCustomer.Id;
                    _customerService.UpdateBillingAddress(address);
                }
                else
                {
                    ...
                    if (address == null)
                    {
                        ...
                        _workContext.CurrentCustomer.Addresses.Add(address);
                        address.CustomerId = _workContext.CurrentCustomer.Id;
                        address.Address1 += " No:" + model.NewAddress.BuildingNumber;
                        if (!string.IsNullOrEmpty(model.NewAddress.IndoorNumber))
                        {
                            address.Address1 += " Daire: " + model.NewAddress.IndoorNumber;
                        }
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "DistrictName", Value = form["BillingNewAddress.DistrictName"], StoreId = "" });
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "StreetName", Value = form["BillingNewAddress.StreetName"], StoreId = "" });
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "BuildingName", Value = form["BillingNewAddress.BuildingNumber"].ToString().Split(',')[0], StoreId = "" });
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "IndoorNumber", Value = form["BillingNewAddress.IndoorNumber"].ToString().Split(',')[0], StoreId = "" });
                        _customerService.InsertAddress(address);
                    }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (address == null)
                    {
                        ...
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "DistrictName", Value = form["ShippingNewAddress.DistrictName"], StoreId = "" });
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "StreetName", Value = form["ShippingNewAddress.StreetName"], StoreId = "" });
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "BuildingName", Value = form["ShippingNewAddress.BuildingNumber"].ToString().Split(',')[0], StoreId = "" });
                        address.GenericAttributes.Add(new Core.Domain.Common.GenericAttribute() { Key = "IndoorNumber", Value = form["ShippingNewAddress.IndoorNumber"].ToString().Split(',')[0], StoreId = "" });
                        _customerService.InsertAddress(address);
                    }
        ...
        public virtual IActionResult OpcSavePaymentMethod(IFormCollection form)
        {
            try
            {
                ...
                //Customer's selected pay money 
                string selectedPaymentPoint = form["paymentPoint"];
                if (!string.IsNullOrEmpty(selectedPaymentPoint))
                    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, "SelectedPaymentPoint", selectedPaymentPoint,
                       _storeContext.CurrentStore.Id);
                else
                    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, "SelectedPaymentPoint", "0",
                    _storeContext.CurrentStore.Id);
        ...
        public ActionResult GetCartPartial()
        {
            var cart = _workContext.CurrentCustomer.ShoppingCartItems
                ...
                .LimitPerStore(_storeContext.CurrentStore.Id)
                .ToList();
            var cartModel = new Models.ShoppingCart.ShoppingCartModel();
            _shoppingCartWebService.PrepareShoppingCart(cartModel, cart);

            return PartialView("_CheckOutCart", cartModel);
        }
