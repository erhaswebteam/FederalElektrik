...
namespace Grand.Web.Areas.Admin.Controllers
{
    public partial class OrderController : BaseAdminController
    {
        ...
        protected virtual void PrepareOrderDetailsModel(OrderModel model, Order order)
        {
            ...
            model.IsMobilAdmin = _workContext.CurrentCustomer.CustomerRoles.Any(x => x.Id == "5b1638616f3079107c356c9d");
            ...
            int? paymentStatusId = null, int? shippingStatusId = null, DateTime? startDate = null)
        {
            ...
            model.IsMobilAdmin = _workContext.CurrentCustomer.CustomerRoles.Any(x => x.SystemName == "MobilAdmin");
        ...
        public IActionResult OrderList(DataSourceRequest command, OrderListModel model)
        {
            ...
            var orders = _orderService.SearchOrders(storeId: model.StoreId,
                ...
                pageSize: command.PageSize, name: model.Name, companyName: model.CompanyName, RoleName: model.RoleName);
        ...
        [HttpPost, ActionName("List")]
        [FormValueRequired("exportbasfexcel-all")]
        public IActionResult ExportBasfProductExcelAll(OrderListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageOrders))
                return AccessDeniedView();
            ...
            var orders = _orderService.SearchOrders(storeId: model.StoreId,
                ...
                billingLastName: model.BillingLastName,
                billingCountryId: model.BillingCountryId,
                orderGuid: model.OrderGuid, name: model.Name, companyName: model.CompanyName);
            ...
            {
                foreach (var prdk in item.OrderItems)
                {
                    string _sku = string.Empty;
                    var _tempProduct = _productService.GetProductById(prdk.ProductId);
                    _sku = _tempProduct.FormatSku(prdk.AttributesXml, _productAttributeParser);

                    ProductOrderModel _orderProduct = new ProductOrderModel();
                    _orderProduct.BirimFiyati = prdk.UnitPriceInclTax;
                    _orderProduct.OrderNo = item.OrderNumber.ToString();
                    _orderProduct.ProductName = _productService.GetProductById(prdk.ProductId).Name;
                    _orderProduct.Quantity = prdk.Quantity;
                    _orderProduct.ShipmentAddress = item.ShippingAddress.Address1;
                    _orderProduct.Total = (prdk.UnitPriceInclTax * prdk.Quantity);
                    _orderProduct.Username = _customerService.GetCustomerById(item.CustomerId).Username;
            ...
            try
            {
                byte[] bytes = _exportManager.ExportOrderProductToXlsx(orderProductList);
                return File(bytes, "text/xls", "orders.xlsx");
            }
            catch (Exception exc)
            {
        ...
        [HttpPost, ActionName("List")]
        [FormValueRequired("exportexcelforlogo-all")]
        public IActionResult ExportExcelForLogoAll(OrderListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageOrders))
                return AccessDeniedView();
            ...
            foreach (var item in orders)
            {
                foreach (var prdk in item.OrderItems)
                {
                    ...
                    ProductOrderModel _orderProduct = new ProductOrderModel();
                    _orderProduct.TaxRate = _taxCategoryService.GetTaxCategoryById(_tempProduct.TaxCategoryId).Name;
                    _orderProduct.BirimFiyati = prdk.UnitPriceExclTax;
                    _orderProduct.OrderNo = item.OrderNumber.ToString();
                    _orderProduct.ProductName = _productService.GetProductById(prdk.ProductId).Name;
                    _orderProduct.Quantity = prdk.Quantity;
                    _orderProduct.ShipmentName = item.ShippingAddress.FirstName;
                    _orderProduct.ShipmentSurname = !string.IsNullOrEmpty(item.LastName) ? item.LastName : string.Empty;
                    _orderProduct.ShipmentAddress = item.ShippingAddress.Address1;
                    _orderProduct.ShipmentCity = !string.IsNullOrEmpty(item.ShippingAddress.CountryId) ? _countryService.GetCountryById(item.ShippingAddress.CountryId).Name : string.Empty;
                    _orderProduct.ShipmentTown = !string.IsNullOrEmpty(item.ShippingAddress.StateProvinceId) ? _stateProvinceService.GetStateProvinceById(item.ShippingAddress.StateProvinceId).Name : string.Empty;
                    _orderProduct.Phone = !string.IsNullOrEmpty(item.ShippingAddress.PhoneNumber) ? item.ShippingAddress.PhoneNumber : string.Empty;
                    _orderProduct.Total = (prdk.UnitPriceExclTax * prdk.Quantity);
                    _orderProduct.Username = _customerService.GetCustomerById(item.CustomerId).Username;
        ...
        [HttpPost, ActionName("List")]
        [FormValueRequired("exportexcelforadminreport")]
        public IActionResult ExportExcelForAdminReport(OrderListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageOrders))
                return AccessDeniedView();
            ...
            foreach (var item in orders)
            {
                var orderPayments = _orderPaymentService.GetOrderByOrderId(item.Id);
                Customer cus = _customerService.GetCustomerById(item.CustomerId);
                foreach (var prdk in item.OrderItems)
                {
                    string _sku = string.Empty;
                    var _tempProduct = _productService.GetProductById(prdk.ProductId);
                    ...
                    {
                        _tempProduct = _productService.GetDeletedProduct(prdk.ProductId);
                    }
                    _sku = _tempProduct.FormatSku(prdk.AttributesXml, _productAttributeParser);
                    ...
                    ProductOrderModel _orderProduct = new ProductOrderModel();
                    _orderProduct.OrderGuidNo = item.OrderGuid.ToString();
                    _orderProduct.OrderNo = item.OrderNumber.ToString();
                    _orderProduct.ProductName = _tempProduct.Name;
                    _orderProduct.Quantity = prdk.Quantity;
                    _orderProduct.BirimFiyati = prdk.UnitPriceInclTax;
                    _orderProduct.TotalIncPrice = prdk.PriceInclTax.ToString();
                    _orderProduct.Username = cus.Username;
                    _orderProduct.Email = cus.Email;
                    _orderProduct.CompanyName = cus.Addresses.FirstOrDefault()?.FirstName + " " + cus.Addresses.FirstOrDefault()?.LastName;
                ...
                _cargoOrderProduct.TaxRate = "18";
                _cargoOrderProduct.BirimFiyati = Math.Round(item.OrderShippingInclTax / 1.18M, 2);
                _cargoOrderProduct.OrderNo = item.OrderNumber.ToString();
                _cargoOrderProduct.ProductName = "Kargo Ücreti";
                _cargoOrderProduct.Quantity = 1;
                _cargoOrderProduct.ShipmentName = item.ShippingAddress.FirstName;
                _cargoOrderProduct.ShipmentSurname = !string.IsNullOrEmpty(item.ShippingAddress.LastName) ? item.ShippingAddress.LastName : string.Empty;
                _cargoOrderProduct.ShipmentAddress = item.ShippingAddress.Address1;
                _cargoOrderProduct.ShipmentCity = !string.IsNullOrEmpty(item.ShippingAddress.CountryId) ? _countryService.GetCountryById(item.ShippingAddress.CountryId).Name : string.Empty;
                _cargoOrderProduct.ShipmentTown = !string.IsNullOrEmpty(item.ShippingAddress.StateProvinceId) ? _stateProvinceService.GetStateProvinceById(item.ShippingAddress.StateProvinceId).Name : string.Empty;
                _cargoOrderProduct.Phone = !string.IsNullOrEmpty(item.ShippingAddress.PhoneNumber) ? item.ShippingAddress.PhoneNumber : string.Empty;
                _cargoOrderProduct.Total = item.OrderShippingExclTax;
                _cargoOrderProduct.TotalIncPrice = item.OrderShippingInclTax.ToString().Replace(".", ",");
                _cargoOrderProduct.Username = _customerService.GetCustomerById(item.CustomerId).Username;
                _cargoOrderProduct.CariKod = "";
                //_orderProduct.CariKod = _customerService.GetCustomerById(item.CustomerId).GenericAttributes.Where(x => x.Key == "CariKod").FirstOrDefault().Value;
                _cargoOrderProduct.SKU = "KB";
                _cargoOrderProduct.CreatedDate = item.CreatedOnUtc.ToString(string.Format("dd/MM/yyyy HH:mm:ss"));
                _cargoOrderProduct.CompanyName = cus.Addresses.FirstOrDefault()?.FirstName + " " + cus.Addresses.FirstOrDefault()?.LastName;
                orderProductList.Add(_cargoOrderProduct);
                if (orderPayments != null)
                {
                    ProductOrderModel _cargoOrderProductPP = new ProductOrderModel();
                    _cargoOrderProductPP.TaxRate = "18";
                    _cargoOrderProductPP.BirimFiyati = item.OrderTotal-(orderPayments.TotalAmount*100);
                    _cargoOrderProductPP.OrderNo = item.OrderNumber.ToString();
                    _cargoOrderProductPP.ProductName = "Puan";
                    _cargoOrderProductPP.Quantity = 1;
                    _cargoOrderProductPP.ShipmentName = item.ShippingAddress.FirstName;
                    _cargoOrderProductPP.ShipmentSurname = !string.IsNullOrEmpty(item.ShippingAddress.LastName) ? item.ShippingAddress.LastName : string.Empty;
                    _cargoOrderProductPP.ShipmentAddress = item.ShippingAddress.Address1;
                    _cargoOrderProductPP.ShipmentCity = !string.IsNullOrEmpty(item.ShippingAddress.CountryId) ? _countryService.GetCountryById(item.ShippingAddress.CountryId).Name : string.Empty;
                    _cargoOrderProductPP.ShipmentTown = !string.IsNullOrEmpty(item.ShippingAddress.StateProvinceId) ? _stateProvinceService.GetStateProvinceById(item.ShippingAddress.StateProvinceId).Name : string.Empty;
                    _cargoOrderProductPP.Phone = !string.IsNullOrEmpty(item.ShippingAddress.PhoneNumber) ? item.ShippingAddress.PhoneNumber : string.Empty;
                    _cargoOrderProductPP.Total = item.OrderTotal - (orderPayments.TotalAmount * 100);
                    _cargoOrderProductPP.TotalIncPrice = (item.OrderTotal - (orderPayments.TotalAmount * 100)).ToString().Replace(".", ",");
                    _cargoOrderProductPP.Username = _customerService.GetCustomerById(item.CustomerId).Username;
                    _cargoOrderProductPP.CariKod = "";
                    //_orderProduct.CariKod = _customerService.GetCustomerById(item.CustomerId).GenericAttributes.Where(x => x.Key == "CariKod").FirstOrDefault().Value;
                    _cargoOrderProductPP.SKU = "P";
                    _cargoOrderProductPP.CreatedDate = item.CreatedOnUtc.ToString(string.Format("dd/MM/yyyy HH:mm:ss"));
                    _cargoOrderProductPP.CompanyName = cus.Addresses.FirstOrDefault()?.FirstName + " " + cus.Addresses.FirstOrDefault()?.LastName;
                    orderProductList.Add(_cargoOrderProductPP);
                    ProductOrderModel _cargoOrderProductKK = new ProductOrderModel();
                    _cargoOrderProductKK.TaxRate = "18";
                    _cargoOrderProductKK.BirimFiyati = orderPayments.TotalAmount*100;
                    _cargoOrderProductKK.OrderNo = item.OrderNumber.ToString();
                    _cargoOrderProductKK.ProductName = "Kredi Kartı";
                    _cargoOrderProductKK.Quantity = 1;
                    _cargoOrderProductKK.ShipmentName = item.ShippingAddress.FirstName;
                    _cargoOrderProductKK.ShipmentSurname = !string.IsNullOrEmpty(item.ShippingAddress.LastName) ? item.ShippingAddress.LastName : string.Empty;
                    _cargoOrderProductKK.ShipmentAddress = item.ShippingAddress.Address1;
                    _cargoOrderProductKK.ShipmentCity = !string.IsNullOrEmpty(item.ShippingAddress.CountryId) ? _countryService.GetCountryById(item.ShippingAddress.CountryId).Name : string.Empty;
                    _cargoOrderProductKK.ShipmentTown = !string.IsNullOrEmpty(item.ShippingAddress.StateProvinceId) ? _stateProvinceService.GetStateProvinceById(item.ShippingAddress.StateProvinceId).Name : string.Empty;
                    _cargoOrderProductKK.Phone = !string.IsNullOrEmpty(item.ShippingAddress.PhoneNumber) ? item.ShippingAddress.PhoneNumber : string.Empty;
                    _cargoOrderProductKK.Total = orderPayments.TotalAmount*100;
                    _cargoOrderProductKK.TotalIncPrice = (orderPayments.TotalAmount*100).ToString().Replace(".", ",");
                    _cargoOrderProductKK.Username = _customerService.GetCustomerById(item.CustomerId).Username;
                    _cargoOrderProductKK.CariKod = "";
                    //_orderProduct.CariKod = _customerService.GetCustomerById(item.CustomerId).GenericAttributes.Where(x => x.Key == "CariKod").FirstOrDefault().Value;
                    _cargoOrderProductKK.SKU = "KK";
                    _cargoOrderProductKK.CreatedDate = item.CreatedOnUtc.ToString(string.Format("dd/MM/yyyy HH:mm:ss"));
                    _cargoOrderProductKK.CompanyName = cus.Addresses.FirstOrDefault()?.FirstName + " " + cus.Addresses.FirstOrDefault()?.LastName;
                    orderProductList.Add(_cargoOrderProductKK);
                }
            }
        ...
        private string OrderStatusName(int orderStatusId)
        {
            string result = "";
        ...
        private string ShipmentStatusName(int shipmentStatusId)
        {
            string result = "";
            switch (shipmentStatusId)
            {
                case 40:
                    ...
                    break;
                case 30:
                    result = "Kargoya Teslim Edildi";
                    break;
                case 25:
                    result = "Parçalı Gönderi Yapıldı";
                    break;
                case 20:
                    result = "Henüz Kargolanmadı";
                    break;
                case 10:
                    result = "Kargo Gerekli Değil";
                    break;
            }
            return result;
        }

        private string OrderPaymentName(string paymentSystemName)
        {
            string result = "";
        ...
        protected virtual string GetCustomerRolesNames(IList<CustomerRole> customerRoles, string separator = ",")
        {
            ...
            {
                if (customerRoles[i].Name != "Registered")
                {
        ...
        public IActionResult ShipmentListSelect(DataSourceRequest command, ShipmentListModel model)
        {
            ...
            var gridModel = new DataSourceResult
            {
                Data = shipments.Select(shipment => PrepareShipmentModel(shipment, false)).OrderByDescending(x => x.OrderNumber),
