...
namespace Grand.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        #region Products
        public virtual ProductDeleted GetDeletedProduct(string id)
        {
            var deletedProducts = _productDeletedRepository.Table.Where(x=>x.Id==id).FirstOrDefault();
        ...
        /// <param name="productIds">Product identifiers</param>
        /// <returns>Products</returns>
        public virtual IList<Product> GetAllPublishedProducts(string customerId, string storeId, int productsAlsoPurchasedNumber)
        {
            var balance = _rewardPointsService.GetRewardPointsBalance(customerId, storeId);

            var builder = Builders<Product>.Filter;
            var filter = builder.Where(x => x.Published == true && x.Price < balance);
            var products = _productRepository.Collection.Find(filter).ToListAsync().Result;

            return products.OrderBy(prod => Guid.NewGuid()).Take(productsAlsoPurchasedNumber).ToList();
        }
        //Erkan özelleþtirdi son...
        ...
        public virtual void UpdateProductAdmin(Product product)
        {
        ...
        public virtual void AdjustInventory(Product product, int quantityToChange, string attributesXml = "", string warehouseId = "")
        {
            ...
            if (product.ManageInventoryMethod == ManageInventoryMethod.ManageStockByAttributes)
            {
                ...
                if (combination != null)
                {
                    ...
                    //send email notification
                    if (quantityToChange < 0 && combination.StockQuantity == (combination.NotifyAdminForQuantityBelow-1))
                    {
