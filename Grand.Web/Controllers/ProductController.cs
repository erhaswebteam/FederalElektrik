    ...
    public partial class ProductController : BasePublicController
    {
        ...
        public virtual IActionResult ProductDetails(string productId, string updatecartitemid = "")
        {
            ...
            if (product.Published == true)
            {
                ...
                {
                    var genattr = product.GenericAttributes.Where(x => x.Key == it.Sku + "_below").FirstOrDefault();
                    var genattrstr = genattr == null ? "false" : genattr.Value;
                    if (it.NotifyAdminForQuantityBelow > 1 && (it.StockQuantity>0 && it.StockQuantity <= it.NotifyAdminForQuantityBelow && genattrstr == "false"))
                    {
                        it.ProductId = product.Id;
                        _workflowMessageService.SendQuantityBelowStoreOwnerNotification(it, _storeContext.CurrentStore.DefaultLanguageId);
                        _genericAttributeService.SaveAttribute(product, it.Sku + "_below", "true", "");
                    }
                }
            }
