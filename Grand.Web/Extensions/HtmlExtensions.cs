...
namespace Grand.Web.Extensions
{
    public static class HtmlExtensions
    {
        ...
        public static bool CheckKvkk()
        {
            GC.Collect();
            bool result = false;
            var workContext = EngineContext.Current.Resolve<IWorkContext>();
            if (!string.IsNullOrEmpty(workContext.CurrentCustomer.Username))
            {
                var isKvkk = workContext.CurrentCustomer.GenericAttributes.Where(x => x.Key == "KvkkValues");
                if (isKvkk != null && isKvkk.Count() > 0)
                    result = true;
            }
            else if (workContext.CurrentCustomer.CustomerRoles.Any(x => x.SystemName == "Guests"))
            {
                result = true;
            }
            return result;
        ...
        public static bool CheckChangePassword()
        {
            GC.Collect();
            bool result = false;
            var workContext = EngineContext.Current.Resolve<IWorkContext>();
            if (!string.IsNullOrEmpty(workContext.CurrentCustomer.Username))
            {
                if (workContext.CurrentCustomer.CustomerRoles.Any(x => x.SystemName == "Guests"))
                {
                    result = false;
                }
                else
                {
                    var isKvkk = workContext.CurrentCustomer.GenericAttributes.Where(x => x.Key == "IsChangePass");
                    if (isKvkk != null && isKvkk.Count() > 0)
                        result = true;
                }
            }
            else if (workContext == null || workContext.CurrentCustomer == null || string.IsNullOrEmpty(workContext.CurrentCustomer.Username))
            {
                result = true;
            }
            return result;
        ...
        public static string ReplaceTurkish(string text)
        {
            return text.ToLower().Replace("I", "i").Replace("Ş", "ş").Replace("Ü", "ü").Replace("Ö", "ö");
        }

        public static bool IsMobile()
        {
            var httpContext = EngineContext.Current.Resolve<IHttpContextAccessor>();
            var UserAgent = httpContext.HttpContext.Request.Headers["User-Agent"].FirstOrDefault();
            var _isMobile = UserAgent.Contains("iPhone") || UserAgent.Contains("Windows Phone") || UserAgent.Contains("Android") || UserAgent.Contains("iPad");
            if (_isMobile)
                return true;
            else
                return false;
        ...
        public static bool ShowMyDealer()
        {
            var workContext = EngineContext.Current.Resolve<IWorkContext>();
            if (workContext.CurrentCustomer.CustomerRoles.Any(x => x.SystemName == "PuanAktaran" ))
                return true;
            else
                return false;
        }

        public static string GetMySystemRole()
        {
            var workContext = EngineContext.Current.Resolve<IWorkContext>();
            if (workContext.CurrentCustomer.CustomerRoles.Any(x => x.SystemName == "TKSatisMuduru" || x.SystemName == "TK31" || x.SystemName == "TK32" || x.SystemName == "TK33" || x.SystemName == "TK34" || x.SystemName == "TK40" || x.SystemName == "Filo" || x.SystemName == "IsGelistirme" || x.SystemName == "CIS" && x.SystemName == "PuanAktaran"))
               return workContext.CurrentCustomer.CustomerRoles.Where(x => x.SystemName == "TKSatisMuduru" || x.SystemName == "TK31" || x.SystemName == "TK32" || x.SystemName == "TK33" || x.SystemName == "TK34" || x.SystemName == "TK40" || x.SystemName == "Filo" || x.SystemName == "IsGelistirme" || x.SystemName == "CIS").FirstOrDefault()?.SystemName;
            else
                return string.Empty;
        }
    }
