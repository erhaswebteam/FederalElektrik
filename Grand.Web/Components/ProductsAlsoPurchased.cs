...
namespace Grand.Web.Components
{
    public class ProductsAlsoPurchasedViewComponent : ViewComponent
    {
        ...
        #region Invoker
        /*
         * //Orjinali bu. Birlikte satın alınan ürünleri getiriyor. Biz ise puanı yeten ürünlerden random seçeceğiz...
        ...
        */
        ...
        //Kullanıcını puanının yetebildiği ürünler rastgele gelsin istendi...
        public IViewComponentResult Invoke(string productId, int? productThumbPictureSize)
        {
            if (!_catalogSettings.ProductsAlsoPurchasedEnabled)
                return Content("");
            ...
            //load products
            var products = _productService.GetAllPublishedProducts(_workContext.CurrentCustomer.Id, _storeContext.CurrentStore.Id, _catalogSettings.ProductsAlsoPurchasedNumber);
            //ACL and store mapping
            products = products.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
            //availability dates
            products = products.Where(p => p.IsAvailable()).ToList();
