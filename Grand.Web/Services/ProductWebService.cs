...
namespace Grand.Web.Services
{
    public partial class ProductWebService: IProductWebService
    {
        ...
        public ProductWebService(IPermissionService permissionService, IWorkContext workContext, IStoreContext storeContext,
            ...
            CaptchaSettings captchaSettings, LocalizationSettings localizationSettings, IGenericAttributeService genericAttributeService)
        {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            foreach (var attribute in productAttributeMapping)
            {
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        var stok = false;
                        foreach (var it in product.ProductAttributeCombinations)
                        {
                            if (it.AttributesXml.Contains(attributeValue.Id) && it.StockQuantity == 0)
                            {
                                stok = true;
                                break;
                            }
            ...
            {
                var prdattr = product.GenericAttributes.Where(x => x.Key == product.Sku + "_below").FirstOrDefault();
                var prdattrstr = (prdattr == null ? "false" : prdattr.Value);
                if (product.NotifyAdminForQuantityBelow>1 && (product.GetTotalStockQuantity(warehouseId: "") <= product.NotifyAdminForQuantityBelow && prdattrstr=="false"))
                {
                    _workflowMessageService.SendQuantityBelowStoreOwnerNotification(product, _storeContext.CurrentStore.DefaultLanguageId);
                    _genericAttributeService.SaveAttribute(product, product.Sku + "_below", "true");
                }
                else
                {
                    _genericAttributeService.SaveAttribute(product, product.Sku + "_below", "false");
                }
            }
            #endregion 
