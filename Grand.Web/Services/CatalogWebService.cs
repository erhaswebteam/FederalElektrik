...
namespace Grand.Web.Services
{
    public partial class CatalogWebService: ICatalogWebService
    {
        ...
        private ManufacturerNavigationModel GetManufacturesByCategoryId(IList<string> searchCategoryIds, string currentManufacturerId)
        {
            var languageId = _workContext.WorkingLanguage.Id;
            ManufacturerNavigationModel model = new ManufacturerNavigationModel();
            ...
            {
                foreach (var prdk in products)
                {
                    var prdkManufac = prdk.ProductManufacturers;
                    if (prdkManufac != null && prdkManufac.Count() > 0)
                    {
                        foreach (var manufac in prdkManufac)
                        {
                            ManufacturerBriefInfoModel _isHave = model.Manufacturers.Where(x => x.Id == manufac.ManufacturerId).FirstOrDefault();
                            if (_isHave == null)
                            {
                                var _tempManu = _manufacturerService.GetManufacturerById(manufac.ManufacturerId);
                                var modelMan = new ManufacturerBriefInfoModel
                                {
                                    Id = _tempManu.Id,
                                    Name = _tempManu.GetLocalized(x => x.Name, languageId),
                                    SeName = _tempManu.GetSeName(languageId),
                                    IsActive = currentManufacturer != null && currentManufacturer.Id == _tempManu.Id,
                                };
                ...
                model.TotalManufacturers = model.Manufacturers.Count();
            }
        ...
        public virtual ManufacturerNavigationModel PrepareManufacturerNavigation(string currentManufacturerId)
        {
            ...
            else
            {
                ...
                var cacheModel = _cacheManager.Get(cacheKey, () =>
                {
                    ...
                    manufacturers = _manufacturerService.GetAllManufacturers(pageSize: _catalogSettings.ManufacturersBlockItemsToDisplay, storeId: _storeContext.CurrentStore.Id);
                    model = new ManufacturerNavigationModel
                    {
        ...
        public virtual SearchModel PrepareSearch(SearchModel model, CatalogPagingFilteringModel command)
        {
            ...
            if (isSearchTermSpecified)
            {
                ...
                else
                {
                    ...
                    //products
                    products = _productService.SearchProductsTR(
                        categoryIds: categoryIds,
