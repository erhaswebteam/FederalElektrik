...
{
    public partial class CheckoutWebService : ICheckoutWebService
    {
        ...
        public CheckoutWebService(
            ...
            IShoppingCartWebService shoppingCartWebService, IProductService productService,
        ...
        public virtual CheckoutPaymentMethodModel PreparePaymentMethod(IList<ShoppingCartItem> cart, string filterByCountryId)
        {
            ...
            {
                decimal rewardPointsBalance = _rewardPointsService.GetRewardPointsBalance(_workContext.CurrentCustomer.Id, _storeContext.CurrentStore.Id);
            ...
            //{
            //    totalPoint += shoppingItem.
            //}
            var tempCartModel = new ShoppingCartModel();
            _shoppingCartWebService.PrepareShoppingCart(tempCartModel, cart, isEditable: false, prepareAndDisplayOrderReviewData: false);

            foreach (var item in cart)
            {
                var prdk = _productService.GetProductById(item.ProductId);
                foreach (var cat in prdk.ProductCategories)
                {
                    var category = _categoryService.GetCategoryById(cat.CategoryId).ParentCategoryId;
                    if (category == "625fe99602846a0418a0c359")
                    {
                        model.Warnings.Add("AlisverisCeki");
                        break;
                    }
                }
            }
            ...
            model.MoneyPointFactor = int.Parse(setting.Value);

            //Ürün Toplamı
            int totalPoint = 0;
            totalPoint = Convert.ToInt32(_orderTotalCalculationService.GetShoppingCartTotal(cart).Value);

            //foreach (var cartItem in tempCartModel.Items)
            //{
            //    totalPoint += decimal.Parse(cartItem.SubTotal.Replace("₺", "").Replace(" + KDV", ""));
            //}

            //// Kargo Ücretleri
            //decimal? shippingTotal = _orderTotalCalculationService.GetShoppingCartShippingTotal(cart);
            //if (shippingTotal.HasValue)
            //    totalPoint += shippingTotal.Value;

            ////KDV Ücretleri
            //decimal? taxValue = _orderTotalCalculationService.GetTaxTotal(cart);
            //if (taxValue.HasValue)
            //totalPoint += taxValue.Value;

            model.TotalPoint = totalPoint;
