...
{
    public partial class OrderWebService: IOrderWebService
    {
        ...
        public virtual OrderDetailsModel PrepareOrderDetails(Order order)
        {
            ...
            foreach (var orderItem in order.OrderItems)
            {
                ...
                if (_havaleDiscountRateSett != null)
                    model.HavaleDiscountRate = decimal.Parse(_havaleDiscountRateSett.Value);
                decimal _discountTotal = (order.OrderTotal + ((order.OrderTotal * model.HavaleDiscountRate) / 100));
                model.HavaleDiscountTotal = _priceFormatter.FormatPrice(_discountTotal, true, order.CustomerCurrencyCode, false, _workContext.WorkingLanguage);
            }
