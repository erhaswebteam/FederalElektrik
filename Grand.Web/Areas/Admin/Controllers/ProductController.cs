    ...
    public partial class ProductController : BaseAdminController
    {
        ...
        public IActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                SaveProductWarehouseInventory(product, model);
                //picture seo names
                if (!_mediaSettings.CdnMediaServerIsActive)
                {
        ...
        public IActionResult ExportExcelAll(ProductListModel model)
        {
            ...
            var productList = _productService.SearchProducts(
                categoryIds: categoryIds,
                manufacturerId: model.SearchManufacturerId,
                storeId: model.SearchStoreId,
                vendorId: model.SearchVendorId,
            ...
            List<ProductExportModel> prdkExportModel = new List<ProductExportModel>();
            foreach (var prdk in productList)
            {
                var cats = "";
                ...
                var attvalues = prdk.ProductAttributeMappings.ToList();
                var vals = new List<string>();
                if (attvalues != null && attvalues.Count > 0)
                {
                    foreach (var it in attvalues)
                    {
                        foreach (var tt in it.ProductAttributeValues)
                        {
                            vals.Add(tt.Name);
                        }
                    }
                }
                var att = prdk.ProductAttributeCombinations.ToList();
                ...
                if (att != null && att.Count > 0)
                {
                    ...
                    foreach (var item in att)
                    {
                        ...
                        var cusRoles = prdk.CustomerRoles.ToList();
                        if (cusRoles != null && cusRoles.Count() > 0)
                        {
                            for (int i = 0; i < cusRoles.Count; i++)
                            {
                                var _role = _customerService.GetCustomerRoleById(cusRoles[i]);
                                if (!_role.IsSystemRole)
                                {
                                    roles.Append(_role.Name);
                                    if (i != cusRoles.Count - 1)
                                    {
                                        roles.Append(",");
                                        roles.Append(" ");
                                    }
                        ...
                        ProductExportModel _tempModel = new ProductExportModel();
                        _tempModel.Role = roles.ToString();
                        _tempModel.ProductName = prdk.Name + " " + vals[index];
                        _tempModel.SKU = item.Sku;
                        _tempModel.Desi = prdk.Weight.ToString();
                        //if (prdk.ManageInventoryMethod == ManageInventoryMethod.DontManageStock)
                        //    _tempModel.Stock = 0;
                        //else
                        //    _tempModel.Stock = item.StockQuantity;
                        _tempModel.Stock = item.StockQuantity;
                        _tempModel.IsActive = prdk.Published ? "AKTİF" : "PASİF";
                        _tempModel.TaxRate = taxRate;
                        //_tempModel.PriceExl = taxRate == "18" ? (prdk.Price / decimal.Parse("1.18")) : (prdk.Price / decimal.Parse("1.08"));
                        _tempModel.PriceInc = prdk.Price;
                        _tempModel.ProductCategory = cats.TrimEnd(';');
                        _tempModel.CreatedDate = prdk.CreatedOnUtc.ToString("dd/MM/yyyy HH:mm");
                        prdkExportModel.Add(_tempModel);
                        index++;
                    }
                }
                else
                {
                    ...
                    ProductExportModel _tempModel = new ProductExportModel();
                    _tempModel.Role = roles.ToString();
                    _tempModel.ProductName = prdk.Name;
                    _tempModel.SKU = prdk.Sku;
                    _tempModel.Desi = prdk.Weight.ToString();
                    //if (prdk.ManageInventoryMethod == ManageInventoryMethod.DontManageStock)
                    //    _tempModel.Stock = 0;
                    //else
                    //    _tempModel.Stock = prdk.StockQuantity;
                    _tempModel.Stock = prdk.StockQuantity;
                    _tempModel.IsActive = prdk.Published ? "AKTİF" : "PASİF";
                    _tempModel.TaxRate = taxRate;
                    //_tempModel.PriceExl = taxRate == "18" ? (prdk.Price / decimal.Parse("1.18")) : (prdk.Price / decimal.Parse("1.08"));
        ...
        [HttpPost, ActionName("List")]
        [FormValueRequired("exportexcel-customerquota")]
        public IActionResult ExportExcelCustomerQuota(ProductListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            var productsList = _productService.SearchProducts(
                categoryIds: categoryIds,
                manufacturerId: model.SearchManufacturerId,
                storeId: model.SearchStoreId,
                vendorId: model.SearchVendorId,
            ...
            string[] customerRoles = { "5b1638616f3079107c356c9f" };

            var customerList0 = _customerService.GetAllCustomers();
            var customerList = customerList0.Where(c => c.Active == true);
            List<CustomerProductQuotaModel> customerQuota = new List<CustomerProductQuotaModel>();
            ...
            foreach (var customer in customerList)
            {
                ...
                {
                    string quota = "0";
                    var _productQuota = customer.GenericAttributes.Where(x => x.Key == product.Id).FirstOrDefault();
                    if (_productQuota != null)
                        quota = _productQuota.Value;

                    CustomerProductQuotaModel _exportModel = new CustomerProductQuotaModel
                    {
                        NameSurname = customer.GetFullName(),
                    ...
                    customerQuota.Add(_exportModel);
                }
            }

            try
            {
                byte[] bytes = _exportManager.ExportCustomerProductsQuotaToXlsx(customerQuota);
                return File(bytes, "text/xls", "Prometeon_Kullanici_Kotalari.xlsx");
            }
            catch (Exception exc)
            {
                ErrorNotification(exc);
        ...
        [HttpPost]
        public IActionResult ImportExcelCustomerQuota(IFormFile customerquotafile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            try
            {
                if (customerquotafile != null && customerquotafile.Length > 0)
                {
                    _importManager.ImportCustomerQuotaFromXlsx(customerquotafile.OpenReadStream());
                }
                else
                {
                    ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
        ...
        [HttpGet]
        public IActionResult updateAcl()
        {
            var catssilver = new string[] { "63174e32e153df2a5c90c5de", "63205c150f819e2630cabb8a", "63205c270f819e2630cac7c1", "63ec9648d642530bacd44def", "63205f3c0f819e2630cb04cd", "63b7bce6ede3c717c8d9c7c1" };
            var allCategories = _categoryService.GetAllCategories(showHidden: true);
            foreach(var cat in allCategories)
            {
                var catids = new List<string>() { cat.Id};
                if (catssilver.IndexOf(cat.Id) == -1)
                {
                    cat.SubjectToAcl = true;
                    cat.CustomerRoles.Clear();
                    cat.CustomerRoles.Add("5b1638616f3079107c356c9d");
                    cat.CustomerRoles.Add("5b1638616f3079107c356c9e");
                    cat.CustomerRoles.Add("63e1ee4c370ccf3a5cf38ad8");
                    cat.SubjectToAcl = true;
                    _categoryService.UpdateCategory(cat);
                    var products = _productService.SearchProducts(categoryIds: catids, showHidden: true);
                    foreach (var prd in products)
                    {
                        prd.SubjectToAcl = true;
                        prd.CustomerRoles.Clear();
                        prd.CustomerRoles.Add("5b1638616f3079107c356c9d");
                        prd.CustomerRoles.Add("5b1638616f3079107c356c9e");
                        prd.CustomerRoles.Add("63e1ee4c370ccf3a5cf38ad8");
                        _productService.UpdateProductAdmin(prd);
                    }
                }
               
            }
            var obj = new { status = "success", message = "Güncelleme işlemi tamamlanmıştır" };
            return Json(obj);
        }
    }
}