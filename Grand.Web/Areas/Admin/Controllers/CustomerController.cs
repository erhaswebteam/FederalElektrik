    ...
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected virtual CustomerModel PrepareCustomerModelForList(Customer customer)
        {
            return new CustomerModel
            {
                ...
                Points = GetUserActualPoint(customer.Username)
            };
        }

        private decimal GetUserActualPoint(string username)
        {
            decimal result = 0;
            result = _processService.GetCustomerActualPoint(username);

            return result;
        }

        private decimal GetUserSpend(string username)
        {
            decimal result = 0;
            result = _processService.GetCustomerSpend(username);

            return result;
        }

        private decimal GetUserCancel(string username)
        {
            decimal result = 0;
            result = _processService.GetCustomerCancelPoint(username);

            return result;
        }

        private decimal GetUserTransferEarn(string username)
        {
            decimal result = 0;
            result = _processService.GetCustomerTransferEarnPoint(username);

            return result;
        }
        private decimal GetUserTransferSpend(string username)
        {
            decimal result = 0;
            result = _processService.GetCustomerTransferSpendPoint(username);

            return result;
        }

        private decimal GetUserEarnPoint(string username)
        {
            decimal result = 0;
            result = _processService.GetCustomerEarnPoint(username);

            return result;
        }

        public IActionResult PointHistory(string cId)
        {
            var model = new List<PointHistoryModel>();
            if (!string.IsNullOrEmpty(cId))
            {
             var pointHistoryList =  _processService.GetAllHistory(cId);
                foreach (var item in pointHistoryList)
                {
                    PointHistoryModel tempModel = new PointHistoryModel();
                    tempModel.Point = item.Point;
                    tempModel.Message = item.Description;
                    tempModel.CreatedDate = item.CreatedOnUtc;
                    model.Add(tempModel);
                }
            }
            return View(model);
        ...
        public IActionResult SendPass(string Id)
        {
            var customer = _customerService.GetCustomerById(Id);
            if (customer == null)
                //No customer found with the specified id
            ...
            SuccessNotification("Giriş bilgileri (kullanıcı adı ve şifre) gönderildi.");

            return RedirectToAction("List");
        }

        [HttpPost, ActionName("List")]
        [FormValueRequired("sendpassword-all")]
        public IActionResult SendPasswordAll()
        {
            var customerList = _customerService.GetAllCustomers().Where(x=>!string.IsNullOrEmpty(x.Email) && !x.Email.Contains("@erhas")).ToList() ;
            ...
            foreach (var cus in customerList)
            {
                try
                {
                    string body = string.Format(msg.Body, cus.Username, cus.Password);
        ...
        public IActionResult SendPassSMS(string Id)
        {
            //var customer = _customerService.GetCustomerById(Id);
            //if (customer == null)
            //    //No customer found with the specified id
            ...
            //msg.Append("Panasonic www.viko-port.com portalina giris bilgileriniz :\n");
            //msg.Append("Kullanici adi : " + customer.Username + "\n");
            //msg.Append("Sifre :" + customer.Password + "\n\n");
            //msg.Append("Viko-Port portalini mutlu ve saglikli gunlerde kullanmanizi dileriz.\n");

            //QueuedSMS sms = new QueuedSMS();
            //sms.CreatedOnUtc = DateTime.UtcNow;
            //sms.GSM = customer.GenericAttributes.Where(x => x.Key == "Phone").FirstOrDefault().Value.Trim().Replace(" ", "");
            //sms.IsSend = false;
            //sms.Message = msg.ToString();
            //sms.SendOnUtc = null;

            //_queuedSMSService.InsertQueuedSMS(sms);

            //SuccessNotification("Giriş bilgileri (kullanıcı adı ve şifre) SMS olarak gönderildi.");
        ...
        public IActionResult ExportExcelAll(CustomerListModel model)
        {
            ...
            {
                cus.SystemName = GetUserActualPoint(cus.Username.Replace("DEL_", "").Replace("del_", "")).ToString("0.##");
                cus.GenericAttributes.Add(new GenericAttribute() { Key = "Earn", Value = GetUserEarnPoint(cus.Username.Replace("DEL_","").Replace("del_","")).ToString("0.##") });
                cus.GenericAttributes.Add(new GenericAttribute() { Key = "Spend", Value = GetUserSpend(cus.Username.Replace("DEL_", "").Replace("del_", "")).ToString("0.##") });
                cus.GenericAttributes.Add(new GenericAttribute() { Key = "Cancel", Value = GetUserCancel(cus.Username.Replace("DEL_", "").Replace("del_", "")).ToString("0.##") });
                cus.GenericAttributes.Add(new GenericAttribute() { Key = "TransferEarn", Value = GetUserTransferEarn(cus.Username.Replace("DEL_", "").Replace("del_", "")).ToString("0.##") });
                cus.GenericAttributes.Add(new GenericAttribute() { Key = "TransferSpend", Value = GetUserTransferSpend(cus.Username.Replace("DEL_", "").Replace("del_", "")).ToString("0.##") });
            }
